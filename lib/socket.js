...
module.exports = function(io) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('connect', function(content) {
      ...
      broadcastUserInfo({ quiet: false, room: null });
    });
    
    // ON: GET-USERS
    socket.on('get-users', function(content) {
      console.log('get-users');
      broadcastUserInfo({ quiet: true, room: null });
    });
    ...
    socket.on('new-game', function(content) {
      ...
      broadcastUserInfo({ quiet: true, room: content.room });
      broadcastUserInfo({ quiet: false, room: null });
    });
    ...
    socket.on('leave-room', function(content) {
      ...
      broadcastUserInfo({ quiet: false, room: null });
      broadcastUserInfo({ quiet: true, room: null });
      broadcastUserInfo({ quiet: false, room: content.room });
    });
    ...
    */
    ...
    // EMIT USER INFO
    function broadcastUserInfo(options) {
