  ...
  io.sockets.on('connection', function(socket) {
    ...
    */
    ...
    function dealCards(room) {
      var toReplace = drawCards(room.deck, _.size(room.selected));
      console.log(room.selected);
      console.log(toReplace);
      _.each(room.selected, function(card) {
        var index = _.indexOf(room.field, card);
        console.log(index);
        if (toReplace) room.field[index] = toReplace.splice(0, 1)[0];
        else room.field.splice(index, 1);
      });
