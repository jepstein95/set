...
module.exports = function(io) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    console.log('User connected: ' + hs.user);
    ...
    */
    ...
    // EMIT USER INFO
    function broadcastUserInfo(global, room) {
      ...
      //database.query('SELECT * FROM users', function(err, rows, fields) {
        ...
        _.each(users, function(user) {
            ...
            else if (!room && !user.room) {
              toSend.push(user.username);
            }
        });

        if (rows) {
          var toRoom = room ? true : false;
          var data = {
            room: toRoom,
            usernames: [],
            nicknames: [],
          ...
          _.each(rows, function(row) { 
            if (_.contains(toSend, row.username)) {
              data.usernames.push(row.username);
              data.nicknames.push(row.nickname);
              data.wins.push(row.wins);
              data.losses.push(row.losses);
            } 
          });
